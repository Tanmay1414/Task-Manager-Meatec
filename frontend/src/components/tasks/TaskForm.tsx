...
const TaskFormComponent = ({ task, onSubmit, onCancel, loading = false }: TaskFormProps) => {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div>
                <label htmlFor="title" className="block text-sm font-semibold theme-text mb-2">
                ...
                <input
                    ...
                    id="title"
                    type="text"
                    className={`w-full px-4 py-3 border-2 rounded-xl transition-all theme-input ${validationErrors.title
            ...
            <div>
                <label htmlFor="description" className="block text-sm font-semibold theme-text mb-2">
                    Details (Optional)
                </label>
                <textarea
                    ...
                    id="description"
                    rows={4}
                    className={`w-full px-4 py-3 border-2 rounded-xl transition-all theme-input ${validationErrors.description
                        ? 'border-red-400 focus:border-red-500'
                        : 'focus:border-indigo-500'
            ...
            <div>
                <label htmlFor="status" className="block text-sm font-semibold theme-text mb-2">
                    Current Status
                </label>
                <select
                    {...register('status')}
                    id="status"
                    className="w-full px-4 py-3 border-2 rounded-xl focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-sm transition-all theme-input"
                >
            ...
            <div className="flex justify-end gap-3 pt-6 border-t theme-border">
                <button
                    type="button"
                    onClick={onCancel}
                    className="px-5 py-2.5 text-sm font-semibold rounded-xl theme-ghost-button border theme-border"
                >
                    Dismiss
                </button>
                <button
