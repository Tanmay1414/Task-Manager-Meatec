...
const TaskItemRow = ({ task, onEdit, onDelete, onToggleStatus }: TaskItemProps) => {
    ...
    return (
        <div className="theme-surface rounded-xl shadow-md p-5 card-hover border-l-4 border-indigo-500 transition-colors duration-300">
            <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-4 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        {task.description && (
                            <p
                                className={`text-sm mb-2 ${isCompleted ? 'text-gray-400' : 'theme-text-muted'
                                    }`}
                            >
                                {task.description}
                            </p>
                ...
                <div className="flex items-start gap-2">
                    <button
                        onClick={() => onEdit(task)}
                        className="p-2 text-indigo-400 theme-icon-button rounded-lg transition-all"
                        title="Modify task"
                    >
                        <svg
                            className="w-5 h-5"
                    ...
                    <button
                        ...
                        className="p-2 text-red-500 theme-icon-button rounded-lg transition-all"
                        title="Remove task"
                    >
                        <svg
                            className="w-5 h-5"
            ...
            {confirmDelete && (
                <div className="mt-4 p-4 theme-danger-card border rounded-xl">
                    <p className="text-sm text-red-200 font-semibold mb-3">Confirm task removal?</p>
                    <div className="flex gap-2">
                        ...
                        <button
                            ...
                            className="px-4 py-2 text-sm font-semibold theme-text bg-transparent border theme-border rounded-lg theme-icon-button transition-all"
                        >
                            Keep It
                        </button>
                    </div>
